...
def get_pose(heatmaps, pafs, params):
    
    heatmaps = heatmaps.transpose((1, 2, 0))
    heatmaps = cv2.resize(heatmaps, (0, 0), fx=params['upsample_ratio'],
        fy=params['upsample_ratio'], interpolation=cv2.INTER_CUBIC)

    pafs = pafs.transpose((1, 2, 0))
    pafs = cv2.resize(pafs, (0, 0), fx=params['upsample_ratio'],
        fy=params['upsample_ratio'], interpolation=cv2.INTER_CUBIC)

    total_keypoints_num = 0
    all_keypoints_by_type = []
    for kpt_idx in range(params['num_kpts']):  # 19th for bg
        total_keypoints_num += extract_keypoints(heatmaps[:, :, kpt_idx], all_keypoints_by_type, total_keypoints_num)

    pose_entries, all_keypoints = group_keypoints(all_keypoints_by_type, pafs)
    scale_h = 1.0 * params['net_h'] * params['upsample_ratio'] / params['stride']
    scale_w = 1.0 * params['net_w'] * params['upsample_ratio'] / params['stride']
    for kpt_id in range(all_keypoints.shape[0]):
        all_keypoints[kpt_id, 0] = all_keypoints[kpt_id, 0] / scale_w  # x [0, 1]
        all_keypoints[kpt_id, 1] = all_keypoints[kpt_id, 1] / scale_h  # y [0, 1]
    current_poses = []
    for n in range(len(pose_entries)):
        if len(pose_entries[n]) == 0:
            continue
        pose_keypoints = np.ones((params['num_kpts'], 2), dtype=np.float) * -1.0
        for kpt_id in range(params['num_kpts']):
        ...
        current_poses.append(pose_keypoints.tolist())
